...
// Request counter for managing multiple simultaneous requests
let activeRequests = 0;

// Request interceptor - Add auth token and show loader
api.interceptors.request.use(
  (config) => {
    ...
    // Show global loader for non-GET requests (POST, PUT, DELETE, PATCH)
    if (config.method && config.method.toUpperCase() !== 'GET') {
      activeRequests++;
      if (window.__globalLoader && activeRequests === 1) {
        const loaderText = getLoaderTextForMethod(config.method, config.url);
        window.__globalLoader.showLoader(loaderText);
      }
    }
  ...
  (error) => {
    ...
    if (window.__globalLoader && activeRequests === 0) {
      window.__globalLoader.hideLoader();
    }
    return Promise.reject(error);
  }
);

// Response interceptor - Handle common errors and hide loader
api.interceptors.response.use(
  (response) => {
    // Hide loader for non-GET requests
  ...
  async (error) => {
    ...
    if (originalRequest.method && originalRequest.method.toUpperCase() !== 'GET') {
      activeRequests = Math.max(0, activeRequests - 1);
      if (window.__globalLoader && activeRequests === 0) {
        window.__globalLoader.hideLoader();
      }
...
/**
 * Get appropriate loader text based on HTTP method and URL
 */
const getLoaderTextForMethod = (method, url) => {
  const upperMethod = method.toUpperCase();
  
  if (url.includes('/validate')) return 'Validating...';
...
// API Service Functions (with automatic loader integration)
